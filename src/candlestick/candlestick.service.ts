...
export class CandlestickService {
    ...
    async startFetchingData() {
        const symbol = 'ethusdt'; // Example symbol
        ...
        const wsPriceUrl = 'wss://fstream.binancefuture.com/ws/ethusdt@markPrice@1s'
    ...
    async getSupertrend() {
        ...
        for (let i = 1; i < inUptrend.length; i++) {
            if (inUptrend[i] && !inUptrend[i - 1]) {
                const newSignal = { signal: 'Buy', open_time: new Date(data[i].t), low: data[i].l};
            ...
            if (!this.lastSignal || this.lastSignal.type !== newSignal.signal || this.lastSignal.date !== newSignal.open_time) {
                ...
                const newSignal = { signal: 'Sell', open_time: new Date(data[i].t), high: data[i].h};
                ...
                if (!existingSignal) {
                    ...
                    try{
                        ...
                        if(signal_type !== "SELL" || this.signal_count >= 1){
                            ...
                            const orderResponse = this.binanceService.placeFullBalanceOrder("ETHUSDT", signal_type);   // Full balance order place
