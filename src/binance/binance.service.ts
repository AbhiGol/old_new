...
export class BinanceService {
  ...
  async getCurrentBTCPrice(): Promise<number> {
    const url = `${this.apiUrl}/ticker/price?symbol=ETHUSDT`;
    try {
  ...
  async placeMarketOrder(symbol: string, side: string, quantity: number) {
    const timestamp = Date.now();
    const queryString = `symbol=${symbol}&side=${side}&type=MARKET&quantity=${quantity.toFixed(1)}&timestamp=${timestamp}`;
    const signature = this.generateSignature(queryString);
